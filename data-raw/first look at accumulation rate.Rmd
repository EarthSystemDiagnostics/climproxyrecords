...
dat <-  shakun.proxies %>% 
  ...
  group_by(Number, ID, Proxy.type, Core, Age) %>%
...
```{r fit_gams, results='hide'}
 dat1 <- plyr::dlply(dat, c("Number", "ID"))
# 
# gamm1 <- gamm(Proxy.depth.m ~ s(Age),
...
if (plot.it){
  par(mfrow = c(1,2))
  plot(a, residuals = TRUE)
  plot(b, residuals = T, col = "Red")
  par(mfrow = c(1,1))  
}
...
Derivs <- shakun.proxies %>% 
  left_join(., select(shakun.metadata, -Core), by = "Number") %>% 
  select(Number, Core, Proxy.type, ID, Geo.cluster, Archive.type) %>% 
  distinct() %>% 
  mutate(ID2 = paste(Number, ID, sep = ".")) %>% 
  left_join(Derivs, ., by = "ID2")
```
...
Derivs.timepoints <- plyr::ldply(gD1.timepoints, Tidy.Deriv) %>% 
  tbl_df() 

names(Derivs.timepoints) <-  sub("Age.", "", names(Derivs.timepoints))

Derivs.timepoints <- Derivs.timepoints %>% 
  mutate(ID2 = paste(Number, ID, sep="."))
